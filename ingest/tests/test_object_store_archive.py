...
@pytest.fixture
def archive():
    # moto's mock_aws() only intercepts recognized AWS hostnames — ObjectStoreArchive
    # always talks to a custom endpoint_url (any S3-compatible provider's whole
    # shape), so that decorator never sees the traffic. A real local moto server,
    # addressed like any other S3-compatible endpoint, exercises the actual HTTP path.
