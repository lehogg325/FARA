...
function Row({ label, count, extra, docIds }: { label: string; count: number; extra?: string | null; docIds: number[] }) {
  const navigate = useStore((s) => s.navigate);
  return (
    <li>
      <div className="row-btn" style={{ flexDirection: "column", alignItems: "flex-start", gap: 3, cursor: "default" }}>
        ...
        <span className="row-meta">
          ...
          {docIds.slice(0, 3).map((id, i) => (
            <span key={id}>
              {i === 0 ? " · " : ", "}
              <button className="back-link" style={{ display: "inline", padding: 0 }} onClick={() => navigate({ kind: "document", id })}>
                filing {id}
              </button>
            </span>
          ))}
...
export function TopEntityList({ countryName }: { countryName: string }) {
  ...
  return (
    <div className="section" style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
        ...
        {hasContacts && (
          <ul className="record-list">
            {(contacts.data as TopContact[]).map((c) => (
              <Row key={c.contact_name_raw} label={c.contact_name_raw} count={c.occurrence_count} docIds={c.sample_registrant_doc_ids} />
            ))}
          </ul>
        )}
      </div>
        ...
        {hasRecipients && (
          <ul className="record-list">
            {(recipients.data as TopRecipient[]).map((r) => (
              <Row key={r.recipient_raw} label={r.recipient_raw} count={r.occurrence_count} extra={money(r.total_amount)} docIds={r.sample_registrant_doc_ids} />
            ))}
          </ul>
        )}
      </div>
