    ...
    return (
      <div className="field-block">
        ...
        <div className="field-text">
          ...
          {field.field_value_numeric !== null && ` — ${formatCurrency(field.field_value_numeric)}`}
...
export function DocumentView({ id }: { id: number }) {
  ...
  const doc = useQuery({ queryKey: ["doc", id], queryFn: ({ signal }) => api.document(id, signal) });
  const fields = useQuery({ queryKey: ["doc-fields", id], queryFn: ({ signal }) => api.documentFields(id, signal) });
  const text = useQuery({ queryKey: ["doc-text", id], queryFn: ({ signal }) => api.documentText(id, signal), retry: false });
  ...
  return (
    <div>
      ...
      <div className="doc-body">
        ...
        {fields.isError && <div className="error-state">Could not load extracted fields.</div>}
        ...
        {!fields.isLoading && !fields.isError && ruleFields.length === 0 && llmFields.length === 0 && (
